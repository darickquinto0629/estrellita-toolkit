...
add_action('admin_menu', 'zoom_management_function');

function zoom_management_function() {
	add_management_page('Zoom Webinars', 'Zoom Webinars', 'manage_woocommerce', 'zoom-registration', 'zoom_webinar_management_page');
}
function zoom_webinar_management_page() {
...
?>
	<div class="wrap">
		...
		<p>East: Daniela</p>
		<hr>
		<?php echo date('Y-m-d', $currentTime); ?>
		<?php



		$pdProducts = array(79351, 2618, 2620, 2624, 64375, 79361);

		foreach ($pdProducts as $product_id) {

			echo '<h3><a href="https://estrellita.com/wp-admin/post.php?post='.$product_id.'&action=edit" target="_blank">'.get_the_title($product_id).'</a></h3>';
			...
			foreach($variations as $var_post) {
				...
				$isVirt = '';
				$varpost_id = $var_post->ID;
				$item = wc_get_product($varpost_id);

				$product = get_product($varpost_id);

				$productDateName = get_post_meta($varpost_id, 'attribute_date', true);
				$varSku = get_post_meta($varpost_id, '_sku', true);
				$var_desc = get_post_meta($varpost_id, '_variation_description', true);
				...
				$productDateStr = explode('(', $productDateName);
					...
					if ($productTime > $currentTime) {
						...
						echo '<strong><span style="color:#757575;">ID:</span> <span style="color:#0000000;">'.$varSku.'</span> | <span style="color:#757575;">region</span> '.$var_desc.' - <span  style="'.$isVirt.'">'.get_post_meta($varpost_id, 'attribute_date', true).'</span> </strong>';

					} else {
						$isVirt = 'color:#cccccc;';
						echo '<em style="'.$isVirt.'">off sale: '.get_post_meta($varpost_id, 'attribute_date', true).'</em>';
				...
				jQuery('#responseTable > tbody  > tr').each(function(index, tr) { 
					if (index < 99) {
						jQuery('#userrow-'+index+'').find("td:eq(4)").text('registering');
					 		var registerData = {
					 			'action': 'zoom_ajax_register_bulk_user',
...
add_action('wp_ajax_zoom_ajax_register_bulk_user', 'bulk_regsiter_single_user_callback');
...
function bulk_regsiter_single_user_callback() {
	...
	// $trainingResponse = $gtw->zoom_register_user($trainingID, $userInfo);
